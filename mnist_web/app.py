...
def api_visualize_model_weights():
    ...
    try:
        ...
        logger.info(f"接收到模型权重可视化请求: model_id={model_id}")
        
        # 加载模型
        if model_id:
            # 检查模型注册表是否已加载
            if not model_registry:
                logger.warning("模型注册表为空，尝试重新加载")
                load_model_registry()
                
            # 检查模型ID是否存在于注册表中
            if model_id not in model_registry:
                logger.error(f"找不到模型: {model_id}，可用模型: {list(model_registry.keys())}")
            ...
            logger.info(f"正在加载模型: {model_id}，路径: {model_registry[model_id]['best_model_path']}")
            model_path = model_registry[model_id]['best_model_path']
            model = load_specific_model(model_path)
            
            if not model:
                logger.error(f"无法加载模型文件: {model_path}")
                return jsonify({
                    'status': 'error', 
                    'message': f'Could not load model file for {model_id}'
                })
        else:
            # 使用当前模型
            if current_model is None:
                logger.error("当前没有可用模型")
                return jsonify({
                    'status': 'error', 
                    'message': 'No current model available'
                })
            logger.info("使用当前模型")
            model = current_model
