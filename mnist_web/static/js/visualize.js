...
function setupEventListeners() {
    ...
    document.getElementById('model-select').addEventListener('change', function() {
        ...
        console.log(`选择了模型: ${selectedOption.textContent}, ID=${state.currentModel}`);
    });
    ...
    document.getElementById('generate-btn').addEventListener('click', function() {
        console.log('生成可视化，当前选择的模型ID:', state.currentModel);
    ...
    fetch('/api/models')
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                populateModelSelector(modelsObj);
                
                console.log('加载模型列表成功:', Object.keys(modelsObj));
            } else {
...
function populateModelSelector(models) {
    ...
    if (models && Object.keys(models).length > 0) {
        for (const [modelId, model] of Object.entries(models)) {
            ...
            option.value = modelId; // 使用完整的模型ID
            
            // 使用模型的name属性和准确度
            const accuracy = model.performance ? model.performance.accuracy : 0;
            option.textContent = `${model.name || modelId} (Acc: ${(accuracy * 100).toFixed(2)}%)`;
            
            // 调试信息
            console.log(`添加模型选项: ID=${modelId}, 名称=${model.name}, 准确度=${accuracy}`);
...
function generateModelWeights() {
    ...
    console.log(`请求模型权重可视化，使用模型ID: ${state.currentModel}`);
    console.log('可用模型列表:', Object.keys(state.availableModels));
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            console.error('模型权重可视化失败:', data.message);
            
            // 检查是否有可用模型列表
            if (data.available_models && Array.isArray(data.available_models)) {
                let errorMsg = `<div class="error-container">
                    <h3 class="error-title">无法生成模型权重可视化</h3>
                ...
                data.available_models.forEach(modelId => {
                    ...
                    if (model) {
                        errorMsg += `<li>${model.name} (ID: ${modelId})</li>`;
                    } else {
                        errorMsg += `<li>${modelId}</li>`;
                    }
                });
                
                errorMsg += `</ul>
                        <p>请从下拉菜单中选择一个可用的模型，然后重试。</p>
                        <p>当前选择的模型ID是: "${state.currentModel}"</p>
                    </div>
                </div>`;
                
                document.getElementById('visualization-content').innerHTML = errorMsg;
            } else {
